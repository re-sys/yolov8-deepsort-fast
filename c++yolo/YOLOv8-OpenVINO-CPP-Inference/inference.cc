...
void Inference::RunInference(cv::Mat &frame, std::vector<Detection> &results) {
	Preprocessing(frame); // Preprocess the input frame
	inference_request_.infer(); // Run inference
	PostProcessing(frame, results); // Postprocess the inference results
}
...
void Inference::PostProcessing(cv::Mat &frame, std::vector<Detection> &results) {
